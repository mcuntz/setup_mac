...
*** Before starting
    ...
    1. The /Software Update/ takes care of all your accounts, passwords, etc. The step before takes care of all the installed applications. But anything installed from the command line will (most probably) not work anymore. So I first uninstall the all things that will be installed in the steps below, which are Homebrew (including the installed casks), LaTeX, Python/pyenv, and everything installed with install\under{}netcdf. I also set back Apple's bash shell as default (because the newer bash shell is removed together with Homebrew).
       ...
       #+BEGIN_SRC bash
         # emacs temurin panoply quarto motrix
...
** LaTeX
   ...
   #+BEGIN_SRC bash
     ...
     sudo tlmgr install \
         ...
         arydshln subfiles mwe easytable gradientframe pbox floatrow \
...
** Python
   ...
   - *pyenv and pyenv-virtualenv 101*
     ...
     #     --with-tcltk-includes='-I${HOMEBREW_PREFIX}/opt/tcl-tk@8/include' \
     #     --with-tcltk-libs='-L${HOMEBREW_PREFIX}/opt/tcl-tk@8/lib -ltcl8.6 -ltk8.6' \
