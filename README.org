...
*** Before starting
    ...
    1. You might want to write down your /Computer Name/ from /System Preferences/ > /Sharing/ or /System Preferences/ > /General/ > /Sharing/ > /Local hostname/.
...
*** Before starting
    ...
    1. Update all your installed apps in \/Applications. The newest app versions will probably already be suitable for the new macOS version. It avoids the problem that you cannot open the old app version on the new macOS system anymore to do the simple update and then have to uninstall/install the app again. This might also need the licence key, etc.
       ...
       Use the /App Store.../ for all Apple programs.

    1. The /Software Update/ takes care of all your accounts, passwords, etc. The step before takes care of all the installed applications.
...
** Homebrew
   ...
   This installs Homebrew into =/usr/local= on mac OS X Intel and into =/opt/homebrew= on macOS Apple Silicon (M1, M2, etc.).
...
** netcdf4\under{}fortran and openmpi development packages
   ...
   #+BEGIN_SRC bash
     env MPICC=/usr/local/openmpi-4.1.2-gfortran/bin/mpicc python -m pip install mpi4py
   #+END_SRC
