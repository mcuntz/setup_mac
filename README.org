...
** Emacs
   I used to use [[http://aquamacs.org][Aquamacs]], but use [[https://www.spacemacs.org][Spacemacs]] now. The latter is very fast, but startup times are very slow and it has a steep learning curve. At the moment I am not using the two most praised modes: helm and evil. I also needed quite some configuration in the =dotspacemacs/user-config= section of =.spacemacs.= My current .spacemacs is as dot-spacemacs in this repository.
   ...
   #+BEGIN_SRC bash
     ...
     if [[ -f .spacemacs.env ]] ; then mv .spacemacs.env .spacemacs.env.bak ; fi
     if [[ -d .emacs.d ]] ; then
         cd .emacs.d
         git status
         cd ..
   ...
   importmagic and epc for Spacemacs' Python mode. The vim-mode of Spacemacs needs the GNU version of the tar utility, so I also install it immediately with Homebrew even that I do not use vim-mode wittingly:

   #+BEGIN_SRC bash
     brew install aspell importmagic epc gnu-tar
   #+END_SRC

   If you open the new Emacs for the first time, it will install and byte-compile more than 100 packages. This might take some time. But you will be asked at least one confirmation in-between (you can press =y= immediately when Spacemacs starts installing).
...
*** Notes for Spacemacs
   ...
   If org-mode does not work with Emacs 28+, e.g. by telling =File mode specification error: (invalid-function org-assert-version)=, remove the org-layer (here 9.6.7)
